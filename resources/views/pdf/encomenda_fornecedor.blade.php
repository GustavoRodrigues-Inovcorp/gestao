...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .doc-title { font-size: 20px; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; }
        ...
        .date-label { font-size: 9px; font-weight: 700; text-transform: uppercase; color: #999; }
        .date-value { font-size: 12px; font-weight: 600; margin-top: 2px; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        thead tr { background: #111; color: #fff; }
        thead th { padding: 10px 12px; text-align: left; font-size: 10px; }
...
<div class="page">
    ...
    <div class="header">
        ...
        <div class="doc-info">
            <div class="doc-title">Encomenda Fornecedor</div>
    ...
    <div class="parties">
        <div class="party">
            ...
            <div class="party-detail">
                {{ $empresa?->morada }}<br>
                {{ $empresa?->codigo_postal }} {{ $empresa?->localidade }}
            </div>
        </div>
        <div class="party">
            <div class="party-label">Para (Fornecedor)</div>
            <div class="party-name">{{ $encomenda->fornecedor->nome }}</div>
            <div class="party-detail">
                {{ $encomenda->fornecedor->morada }}<br>
                {{ $encomenda->fornecedor->codigo_postal }} {{ $encomenda->fornecedor->localidade }}<br>
                {{ $encomenda->fornecedor->nif ? 'NIF: ' . $encomenda->fornecedor->nif : '' }}
            </div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right">{{ number_format($linha->preco_custo, 2, ',', '.') }} €</td>
    ...
    <div class="footer">{{ $empresa?->nome }} &bull; {{ $empresa?->email }}</div>
</div>
</body>
</html>